...
> SEMPRE pergunte **"Qual a data do relatório?"** E **"Qual o cliente?"** antes de iniciar — não assumir data nem cliente.
> Os clientes disponíveis estão em `CLIENTES/orgs.json` (Onfly, Elven observability, Dotz, Unicred).
> Após as respostas, executar TODO o workflow abaixo de forma autônoma, sem pedir confirmação a cada etapa
...
| `{{MTTR}}` | ex: `00:42:43` |
| `{{MTTA}}` | ex: `00:15:00` ou `N/A` se sem ACK no turno |
...
| `{{EVIDENCIA_MTTA}}` | texto do card MTTA — ex: `MTTA médio de 15min — X eventos reconhecidos` ou `Nenhum ACK registrado no turno` |
...
- MTTR no formato `HH:MM:SS`
- MTTA no formato `HH:MM:SS` ou `N/A` quando nenhum evento tiver ACK no turno — calcular via `fct__eventsHistory` filtrando `action = 'acknowledged'`
